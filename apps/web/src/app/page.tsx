    ...
    <section className={styles.reportShell}>
      ...
      <div className={styles.reportLayout}>
        <div>
          ...
          {Boolean(analytics?.totalWorkflow)&&(()=>{const flow=analytics!.totalWorkflow as Record<string,unknown>;const companyRows=(analytics!.totalCompanies??[]) as Record<string,unknown>[];const lastSuccess=flow.lastSuccessAt?new Date(String(flow.lastSuccessAt)).toLocaleString("fr-FR"):"Jamais";return <section className={styles.totalWorkflowSection}>
            <div className={styles.flowKpiHeading}><div><small>WORKFLOW TOTAL MOBILITY · TOUTES LES SOCIÉTÉS</small><h3>Couverture de l’extraction en temps réel</h3><p>De la source Total jusqu’aux indicateurs de consommation : chaque étape est contrôlée, rapprochée et consolidée par société.</p></div><span className={styles.flowLiveStatus}><i />{String(flow.lastRunStatus??"INCONNU")} · {lastSuccess}</span></div>
            <div className={styles.totalWorkflowSteps}>
              <article><span>01</span><div><small>CLIENTS</small><b>{Number(flow.companies??0)} sociétés</b><em>Sélection et isolation par client Total</em></div><i aria-hidden="true">→</i></article>
              <article><span>02</span><div><small>CARTES</small><b>{Number(flow.cards??0)} cartes</b><em>{Number(flow.freshCards??0)} contrôlées récemment</em></div><i aria-hidden="true">→</i></article>
              <article><span>03</span><div><small>CHAUFFEURS</small><b>{Number(flow.drivers??0)} chauffeurs</b><em>{Number(flow.freshDrivers??0)} synchronisés récemment</em></div><i aria-hidden="true">→</i></article>
              <article><span>04</span><div><small>VÉHICULES & KM</small><b>{Number(flow.vehicles??0)} véhicules</b><em>{Number(flow.trackedVehicles??0)} avec suivi Total/transaction</em></div><i aria-hidden="true">→</i></article>
              <article><span>05</span><div><small>CONSOMMATION</small><b>{Number(flow.transactions??0)} transactions</b><em>{Number(flow.mileageTransactions??0)} avec kilométrage</em></div><i aria-hidden="true">✓</i></article>
            </div>
            <div className={styles.totalCompanyFlow}><div><b>Société</b><b>Cartes</b><b>Véhicules</b><b>Chauffeurs</b><b>Transactions</b><b>Litres</b><b>Montant</b><b>Dernier flux</b></div>{companyRows.map(row=><div key={String(row.code)}><strong>{String(row.code)}</strong><span>{Number(row.cards??0)}</span><span>{Number(row.vehicles??0)}</span><span>{Number(row.drivers??0)}</span><span>{Number(row.transactions??0)}</span><span>{Number(row.liters??0).toLocaleString("fr-FR",{maximumFractionDigits:2})} L</span><span>{Number(row.amount??0).toLocaleString("fr-FR",{maximumFractionDigits:3})} DT</span><span>{row.lastTransactionAt?new Date(String(row.lastTransactionAt)).toLocaleString("fr-FR"):"Aucun flux"}</span></div>)}</div>
            {Boolean(flow.lastError)&&<div className={styles.totalWorkflowError}>Dernière erreur Total : {String(flow.lastError)}</div>}
          </section>})()}
      ...
      <form className={styles.modal} onSubmit={submit}>
        ...
        {type === "cardAction" ? (
          ...
          <div className={styles.formGrid}>
            ...
            {type === "mileage" && <><label className={styles.fullField}>{user.role==="ZIN_FINANCE"?"Véhicule actif de la société sélectionnée":"Véhicule de votre périmètre société"}<select name="vehicleId" required defaultValue=""><option value="" disabled>Sélectionner un véhicule</option>{vehicles.map(vehicle=><option value={String(vehicle.id)} key={String(vehicle.id)}>{String(vehicle.immatriculation)} · dernier relevé {Number(vehicle.kilometrage??0).toLocaleString("fr-FR")} km</option>)}</select></label><label>Nouveau kilométrage<input name="mileage" type="number" min="0" step="0.1" required /></label><label>Observation<input name="note" /></label><div className={styles.workflowInfo}><b>Contrôle automatique par société</b><span>{user.role==="ZIN_FINANCE"?"La saisie Zin est validée immédiatement dans la société sélectionnée et notifiée à la DG. Tout écart génère une anomalie.":"Après la répartition, le responsable saisit le kilométrage dans sa société. Zin ou la DG contrôle ensuite le relevé et tout écart détecté."}</span></div></>}
...
function Settings({ reset,token,user,companyId,companyCode,notify,onSynced }: { reset:()=>void;token:string|null;user:User;companyId:string;companyCode:string;notify:(message:string)=>void;onSynced:()=>void }) {
  ...
  const [agentKind,setAgentKind]=useState<"REALTIME"|"CARD_REFERENCE">("REALTIME");
    ...
    <section className={styles.settings}>
      {direction&&<article className={styles.totalConnector}>
        ...
        <details className={styles.connectorConfig}><summary>Mode de secours administrateur</summary><form onSubmit={connect} className={styles.connectorForm}>
          ...
          <label>Synchronisation automatique<select name="syncIntervalMinutes" defaultValue="1"><option value="1">Temps réel · chaque minute</option></select></label>
