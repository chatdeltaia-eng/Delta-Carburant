...
@Injectable() export class FuelPricesService{constructor(private readonly db:DatabaseService){}
 ...
 async refreshTunisia(actor:{sub:string;email:string}){
  ...
  return this.db.transaction(async client=>{const companies=await client.query<{id:string;code:string}>(`SELECT id,code FROM company WHERE active ORDER BY code`);if(!companies.rows.length)throw new BadRequestException('Aucune société active');let changed=0,imported=0;
   const billingByCompany:Record<string,unknown>={};
   for(const company of companies.rows){
    const before=new Map<string,number>();
    for(const item of latest){const previous=await client.query(`SELECT new_price FROM fuel_price WHERE company_id=$1 AND product=$2 ORDER BY effective_date DESC,created_at DESC LIMIT 1`,[company.id,item.product]);if(previous.rows[0])before.set(item.product,Number(previous.rows[0].new_price));}
    for(const item of history){const previous=await client.query(`SELECT new_price FROM fuel_price WHERE company_id=$1 AND product=$2 AND effective_date<$3::date ORDER BY effective_date DESC,created_at DESC LIMIT 1`,[company.id,item.product,item.effectiveDate]);const old=Number(previous.rows[0]?.new_price??item.price);const variation=old?100*(item.price-old)/old:0;
    ...
    for(const item of latest){const old=before.get(item.product);if(old===undefined||old===item.price)continue;const variation=100*(item.price-old)/old;changed++;
     await client.query(`INSERT INTO notification(user_id,title,message,severity,target_view,entity_type,entity_id) SELECT id,$1,$2,'WARNING','fuelPrices','fuel_price',NULL FROM app_user WHERE active AND (company_id=$3 OR role IN('ZIN_FINANCE','DIRECTION_GENERAL','SUPER_ADMIN'))`,[`Prix carburant ${company.code} ${variation>0?'augmenté':'diminué'}`,`${item.product} : ${old.toFixed(3)} → ${item.price.toFixed(3)} TND/l (${variation.toFixed(2)} %), applicable le ${item.effectiveDate}`,company.id]);}
    billingByCompany[company.code]=await this.recalculateBilling(client,company.id);
   }
   await client.query(`INSERT INTO audit_log(actor,action,entity_type,entity_id,new_values) VALUES($1,'REFRESH_TUNISIA','fuel_price','OFFICIAL',$2)`,[actor.email,{source:this.officialUrl,prices:latest,historyImported:imported,changed,billingByCompany,companies:companies.rows.map(company=>company.code)}]);return {source:this.officialUrl,checked:latest.length*companies.rows.length,changed,prices:latest,historyImported:imported,billingByCompany,companies:companies.rows.map(company=>company.code),scope:'3 carburants réglementés appliqués à chaque société active'};});
 }
