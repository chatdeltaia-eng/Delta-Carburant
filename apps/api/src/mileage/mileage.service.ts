...
export class MileageService {
 ...
 async create(dto:{vehicleId:string;mileage:number;note?:string},actor:Actor){return this.db.transaction(async client=>{
   ...
   const detected=Math.max(0,Number(dto.mileage)-previous),expected=Number(dto.mileage);
   ...
   const anomaly=previous>0&&(detected>10000||(liters>0&&detected===0)||(litersPer100Km!==null&&litersPer100Km>40));
   ...
   if(anomaly) await client.query(`INSERT INTO anomaly(vehicle_id,anomaly_type,severity,status,description,assigned_to)
     VALUES($1,'MILEAGE_MISMATCH','HIGH','OPEN',$2,$3)`,[dto.vehicleId,`Relevé ${dto.mileage} km · distance ${detected} km · ${liters.toFixed(3)} L · ${litersPer100Km?.toFixed(2)??'—'} L/100 km`,actor.sub]);
   ...
   return {...result.rows[0],status:zin?'VALIDATED':'PENDING',expectedMileage:expected,detectedDistance:detected,periodLiters:liters,litersPer100Km,anomaly};
 });}
