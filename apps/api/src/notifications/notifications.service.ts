...
export class NotificationsService {
  ...
  async readAll(userId: string) {
    const rows = await this.db.query(`UPDATE notification SET read_at=now()
      WHERE user_id=$1 AND read_at IS NULL RETURNING id`, [userId]);
    return { updated: rows.length, read: true };
  }
  async unreadAll(userId: string) {
    const rows = await this.db.query(`UPDATE notification SET read_at=NULL
      WHERE user_id=$1 AND read_at IS NOT NULL RETURNING id`, [userId]);
    return { updated: rows.length, read: false };
  }
