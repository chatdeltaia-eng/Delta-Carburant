...
@UseGuards(AuthGuard('jwt'),RolesGuard) @Controller('documents') export class DocumentsController {constructor(private readonly service:DocumentsService){}
 ...
 @Post('return-receipts/:id/restore') @Roles('DIRECTION_GENERAL','ZIN_FINANCE') restoreReturnedCard(@Param('id',ParseUUIDPipe) id:string,@Req() req:{user:Actor}){return this.service.restoreReturnedCard(id,req.user);}
