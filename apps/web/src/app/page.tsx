...
function CardReturnsView({token,user,cards,requests,receipts,decide,refresh,notify}:{token:string;user:User;cards:Card[];requests:Row[];receipts:Row[];decide:(id:string,accepted:boolean)=>void|Promise<void>;refresh:()=>void;notify:(message:string)=>void}){
 ...
 return <section className={styles.fullPanel}><div className={styles.importNotice}><b>Restitution obligatoire à 100 %</b><span>Le plafond et la consommation du mois sont affichés pour contrôle avant les validations de Zin et de la DG.</span></div><div className={styles.tableWrap}><table><thead><tr>{["Carte","Plafond mensuel","Consommation du mois","Utilisation","Najib","Validation Zin","Validation DG","Statut","Reçu / action"].map(label=><th key={label}>{label}</th>)}</tr></thead><tbody>{rows.map(card=>{const request=pendingFor(card),receipt=receiptFor(card);const zinDone=request?.zinValide==='Oui',dgDone=request?.dgValide==='Oui';const alreadyApproved=(user.role==='ZIN_FINANCE'&&zinDone)||(user.role==='DIRECTION_GENERAL'&&dgDone);const limit=Number(receipt?.plafond??card.monthly_limit??0),consumed=Number(receipt?.consomme??card.consumed_amount??limit);return <tr key={card.id}><td><b>{card.masked_card_number}</b><small>{card.beneficiary||'—'} · {card.registration||'—'}</small></td><td><b>{limit.toFixed(3)} TND</b></td><td><b>{consumed.toFixed(3)} TND</b><small>{receipt?`${Number(receipt.litres??0).toFixed(3)} L · ${Number(receipt.transactions??0)} transaction(s)`:"Mois courant"}</small></td><td><span className={styles.documentPending}>{Number(card.consumption_rate??receipt?.taux??0).toFixed(1)} %</span></td><td>{receipt?'✓ Restituée':request?'✓ Demandée':'À restituer'}</td><td>{receipt||zinDone?'✓ Validée':'○ En attente'}</td><td>{receipt||dgDone?'✓ Validée':'○ En attente'}</td><td>{receipt?<span className={styles.documentAuthorized}>✓ Restitution terminée</span>:request?<span className={styles.documentPending}>Validation en cours</span>:<span className={styles.documentPending}>Restitution requise</span>}</td><td>{receipt?<button className={styles.documentPrintBtn} onClick={()=>printReceipt(receipt)}>▣ Reçu preuve</button>:user.role==='NAJIB_ASSIGNER'?<button className={styles.documentApproveBtn} onClick={()=>returnCard(card)}>Restituer la carte</button>:request&&!alreadyApproved?<button className={styles.documentApproveBtn} onClick={()=>decide(String(request.id),true)}>✓ Approuver</button>:<span className={styles.waitingStatus}>{alreadyApproved?'Votre validation est enregistrée':'En attente de Najib'}</span>}</td></tr>})}{!rows.length&&<tr><td colSpan={9}><div className={styles.documentEmpty}>Aucune carte n’a atteint 100 % pour le moment.</div></td></tr>}</tbody></table></div></section>;
}
function DocumentsView({token,returnReceipts,notify}:{token:string;returnReceipts:Row[];notify:(message:string)=>void}){
 ...
 const printReturn=(row:Row)=>printOfficialDocument(`Reçu de restitution ${documentText(row.numero)}`,`<div class="meta"><div><b>N° reçu</b>${documentText(row.numero)}</div><div><b>N° carte</b>${documentText(row.carte)}</div><div><b>Date de restitution</b>${documentText(row.date)}</div><div><b>Heure</b>${documentText(row.heure)}</div></div><div class="summary"><div><span>Plafond mensuel</span><strong>${Number(row.plafond??0).toFixed(3)} TND</strong></div><div><span>Consommation</span><strong>${Number(row.consomme??0).toFixed(3)} TND</strong></div><div><span>Utilisation</span><strong>${Number(row.taux).toFixed(1)} %</strong></div></div><div class="meta"><div><b>Litres consommés</b>${Number(row.litres??0).toFixed(3)} L</div><div><b>Transactions</b>${Number(row.transactions??0)}</div><div><b>Mois concerné</b>${documentText(row.mois)}</div><div><b>Statut</b>Restitution validée</div></div><p>Delta Carburant certifie la restitution physique de la carte après contrôle de sa consommation et validation des trois parties.</p><div class="signatures"><div>Signature Najib<b>${documentText(row.restituePar)}</b></div><div>Signature Zin<b>${documentText(row.recuPar)}</b></div><div>Signature DG<b>${documentText(row.dg)}</b></div></div>`);
