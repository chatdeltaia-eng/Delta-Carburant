...
function Settings({ reset,token,user,notify }: { reset:()=>void;token:string|null;user:User;notify:(message:string)=>void }) {
  ...
  async function connect(event:FormEvent<HTMLFormElement>){event.preventDefault();if(!token)return;setBusy(true);try{const form=new FormData(event.currentTarget);const simplePayload=String(form.get("totalPayload")??"").trim();const copied=simplePayload?readTotalMobilityPayload(simplePayload):{};const configuration={customerId:copied.CustomerId||String(form.get("customerId")??""),customerNumber:copied.CustomerNumber||String(form.get("customerNumber")??""),siteNumber:copied.SiteNumber||String(form.get("siteNumber")??""),userId:copied.UserId||String(form.get("totalUserId")??""),username:copied.usersname||String(form.get("totalUsername")??""),refreshToken:String(form.get("refreshToken")??""),syncIntervalMinutes:Number(form.get("syncIntervalMinutes")??60)};if(!configuration.customerId||!configuration.customerNumber||!configuration.siteNumber)throw new Error("La configuration copiée ne contient pas les informations client Total. Utilisez « Copy value » sur Request Payload.");const response=await fetch(`${API}/total-mobility/connect`,{method:"POST",headers:{Authorization:`Bearer ${token}`,"Content-Type":"application/json"},body:JSON.stringify(configuration)});if(!response.ok){const body=await response.json().catch(()=>null) as {message?:string|string[]}|null;throw new Error(Array.isArray(body?.message)?body.message.join(" · "):body?.message||`Connexion Total refusée (${response.status})`);}event.currentTarget.reset();notify("Total Mobility est connecté. Les identifiants techniques ont été détectés automatiquement.");await loadTotal();}catch(error){notify(error instanceof Error?error.message:"Connexion Total impossible");}finally{setBusy(false);}}
