...
export class CardFollowupService implements OnModuleInit, OnModuleDestroy {
  ...
  async sendDirectionReport(reason = 'MANUAL') {
    ...
    const companies=Object.values(report.rows.reduce<Record<string,{company:string;cards:number;consumed:number;limit:number;critical:number}>>((acc,row)=>{
      const item=acc[row.company]??{company:row.company,cards:0,consumed:0,limit:0,critical:0};
      item.cards++;item.consumed+=row.consumed;item.limit+=row.monthlyLimit;if(row.usageRate>=Number(process.env.CARD_LIMIT_ALERT_PERCENT||90))item.critical++;
      acc[row.company]=item;return acc;
    },{})).sort((a,b)=>a.company.localeCompare(b.company));
    const companyRows=companies.map(row=>`<tr><td><b>${this.escape(row.company)}</b></td><td>${row.cards}</td><td>${row.consumed.toFixed(3)} TND</td><td>${row.limit.toFixed(3)} TND</td><td>${row.limit?Math.round(100*row.consumed/row.limit):0} %</td><td>${row.critical}</td></tr>`).join('');
    const result = await this.mail.send(recipients, `Consommation quotidienne Delta Carburant — ${new Date().toLocaleDateString('fr-TN')}`, `<div style="font-family:Arial,sans-serif;max-width:900px;color:#173b2b"><h2>Consommation quotidienne du groupe</h2><p>Situation actualisée depuis les données Total synchronisées.</p><p><b>${report.totalCards}</b> cartes suivies · <b>${report.criticalCards}</b> cartes proches ou au-dessus du plafond · <b>${report.openAnomalies}</b> anomalies ouvertes.</p><h3>Consommation du mois par société</h3><table style="width:100%;border-collapse:collapse"><thead><tr style="background:#075fae;color:#fff"><th>Société</th><th>Cartes</th><th>Consommation</th><th>Plafond</th><th>Utilisation</th><th>Alertes</th></tr></thead><tbody>${companyRows}</tbody></table><h3>Cartes à surveiller</h3><table style="width:100%;border-collapse:collapse"><thead><tr style="background:#163d2e;color:#fff"><th>Carte</th><th>Société</th><th>Responsable</th><th>Consommation</th><th>Taux</th></tr></thead><tbody>${rows || '<tr><td colspan="5" style="padding:15px">Aucune carte au seuil d’alerte.</td></tr>'}</tbody></table><p style="margin-top:24px"><a href="${this.escape(appUrl)}" style="background:#16856a;color:#fff;padding:12px 18px;border-radius:8px;text-decoration:none">Ouvrir Delta Carburant</a></p></div>`);
