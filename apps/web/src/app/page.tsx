      ...
      <div className={styles.reportLayout}>
        <div>
          ...
          {Boolean(analytics?.totalWorkflow)&&(()=>{const flow=analytics!.totalWorkflow as Record<string,unknown>;const companyRows=(analytics!.totalCompanies??[]) as Record<string,unknown>[];const lastSuccess=flow.lastSuccessAt?new Date(String(flow.lastSuccessAt)).toLocaleString("fr-FR"):"Jamais";return <section className={styles.totalWorkflowSection}>
            <div className={styles.flowKpiHeading}><div><small>WORKFLOW TOTAL MOBILITY · TOUTES LES SOCIÉTÉS</small><h3>Couverture de l’extraction en temps réel</h3></div><span>{String(flow.lastRunStatus??"INCONNU")} · {lastSuccess}</span></div>
            <div className={styles.totalWorkflowSteps}>
              <article><span>01</span><div><small>CLIENTS</small><b>{Number(flow.companies??0)} sociétés</b><em>Sélection et isolation par client Total</em></div></article>
              <article><span>02</span><div><small>CARTES</small><b>{Number(flow.cards??0)} cartes</b><em>{Number(flow.freshCards??0)} contrôlées récemment</em></div></article>
              <article><span>03</span><div><small>CHAUFFEURS</small><b>{Number(flow.drivers??0)} chauffeurs</b><em>{Number(flow.freshDrivers??0)} synchronisés récemment</em></div></article>
              <article><span>04</span><div><small>VÉHICULES & KM</small><b>{Number(flow.vehicles??0)} véhicules</b><em>{Number(flow.trackedVehicles??0)} avec suivi Total/transaction</em></div></article>
              <article><span>05</span><div><small>CONSOMMATION</small><b>{Number(flow.transactions??0)} transactions</b><em>{Number(flow.mileageTransactions??0)} avec kilométrage</em></div></article>
            </div>
